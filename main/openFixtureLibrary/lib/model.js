...
export async function fixtureFromFile(absolutePath) {
  ...
  if (fixtureJson.$schema.endsWith(`/fixture-redirect.json`)) {
    ...
    absolutePath = path.join(
      ...
      `../${manufacturerKey}/${fixtureKey}.json`
    );
...
export async function manufacturerFromRepository(manufacturerKey) {
  const manufacturers = await importJson(
...
async function resolveResourceName(resourceName) {
  ...
  if (remainingParts[0] === `aliases`) {
    ...
    if (!(aliasKey in aliases)) {
      throw new Error(
        `Resource alias '${aliasKey}' not defined in file '${aliasesFilePath}'.`
      );
    }
...
async function getImageForResource(type, baseUrl, key) {
  ...
  if (type === `gobos`) {
    const fileExtensions = resourceFileFormats
      .map(({ extension }) => extension)
      .join(`, `);
    throw new Error(
      `Expected gobo image for resource '${fileURLToPath(
        new URL(key, baseUrl)
      )}' not found (supported file extensions: ${fileExtensions}).`
    );
  }
